...
2007  0284 e600          	ld	a,(_HSIDivFactor,x)
2008  0286 6b09          	ld	(OFST+0,sp),a
2010                     ; 583     clockfrequency = HSI_VALUE / presc;
2012  0288 7b09          	ld	a,(OFST+0,sp)
2013  028a b703          	ld	c_lreg+3,a
2014  028c 3f02          	clr	c_lreg+2
2015  028e 3f01          	clr	c_lreg+1
2016  0290 3f00          	clr	c_lreg
2017  0292 96            	ldw	x,sp
2018  0293 1c0001        	addw	x,#OFST-8
2019  0296 cd0000        	call	c_rtol
2022  0299 ae2400        	ldw	x,#9216
2023  029c bf02          	ldw	c_lreg+2,x
2024  029e ae00f4        	ldw	x,#244
2025  02a1 bf00          	ldw	c_lreg,x
2026  02a3 96            	ldw	x,sp
2027  02a4 1c0001        	addw	x,#OFST-8
2028  02a7 cd0000        	call	c_ludv
2030  02aa 96            	ldw	x,sp
2031  02ab 1c0005        	addw	x,#OFST-4
2032  02ae cd0000        	call	c_rtol
2036  02b1 201c          	jra	L1701
2037  02b3               L7601:
2038                     ; 585   else if ( clocksource == CLK_SOURCE_LSI)
2040  02b3 7b09          	ld	a,(OFST+0,sp)
2041  02b5 a1d2          	cp	a,#210
2042  02b7 260c          	jrne	L3701
2043                     ; 587     clockfrequency = LSI_VALUE;
2045  02b9 aef400        	ldw	x,#62464
2046  02bc 1f07          	ldw	(OFST-2,sp),x
2047  02be ae0001        	ldw	x,#1
2048  02c1 1f05          	ldw	(OFST-4,sp),x
2051  02c3 200a          	jra	L1701
2052  02c5               L3701:
2053                     ; 591     clockfrequency = HSE_VALUE;
2055  02c5 ae2400        	ldw	x,#9216
2056  02c8 1f07          	ldw	(OFST-2,sp),x
2057  02ca ae00f4        	ldw	x,#244
2058  02cd 1f05          	ldw	(OFST-4,sp),x
2060  02cf               L1701:
2061                     ; 594   return((uint32_t)clockfrequency);
2063  02cf 96            	ldw	x,sp
2064  02d0 1c0005        	addw	x,#OFST-4
2065  02d3 cd0000        	call	c_ltor
2069  02d6 5b09          	addw	sp,#9
2070  02d8 81            	ret
2169                     ; 604 void CLK_AdjustHSICalibrationValue(CLK_HSITrimValue_TypeDef CLK_HSICalibrationValue)
2169                     ; 605 {
2170                     	switch	.text
2171  02d9               _CLK_AdjustHSICalibrationValue:
2173  02d9 88            	push	a
...
2181  02da c650cc        	ld	a,20684
2182  02dd a4f8          	and	a,#248
2183  02df 1a01          	or	a,(OFST+1,sp)
2184  02e1 c750cc        	ld	20684,a
2185                     ; 611 }
2188  02e4 84            	pop	a
2189  02e5 81            	ret
...
2213                     ; 623 {
2214                     	switch	.text
2215  02e6               _CLK_SYSCLKEmergencyClear:
2219                     ; 624   CLK->SWCR &= (uint8_t)(~CLK_SWCR_SWBSY);
2221  02e6 721150c5      	bres	20677,#0
2222                     ; 625 }
2225  02ea 81            	ret
2378                     ; 634 FlagStatus CLK_GetFlagStatus(CLK_Flag_TypeDef CLK_FLAG)
2378                     ; 635 {
2379                     	switch	.text
2380  02eb               _CLK_GetFlagStatus:
2382  02eb 89            	pushw	x
2383  02ec 5203          	subw	sp,#3
...
2397  02ee 01            	rrwa	x,a
2398  02ef 9f            	ld	a,xl
2399  02f0 a4ff          	and	a,#255
2400  02f2 97            	ld	xl,a
2401  02f3 4f            	clr	a
2402  02f4 02            	rlwa	x,a
2403  02f5 1f01          	ldw	(OFST-2,sp),x
2404  02f7 01            	rrwa	x,a
2406                     ; 647   if (statusreg == 0x0100) /* The flag to check is in ICKRregister */
2408  02f8 1e01          	ldw	x,(OFST-2,sp)
2409  02fa a30100        	cpw	x,#256
2410  02fd 2607          	jrne	L1421
2411                     ; 649     tmpreg = CLK->ICKR;
2413  02ff c650c0        	ld	a,20672
2414  0302 6b03          	ld	(OFST+0,sp),a
2417  0304 202f          	jra	L3421
2418  0306               L1421:
2419                     ; 651   else if (statusreg == 0x0200) /* The flag to check is in ECKRregister */
2421  0306 1e01          	ldw	x,(OFST-2,sp)
2422  0308 a30200        	cpw	x,#512
2423  030b 2607          	jrne	L5421
2424                     ; 653     tmpreg = CLK->ECKR;
2426  030d c650c1        	ld	a,20673
2427  0310 6b03          	ld	(OFST+0,sp),a
2430  0312 2021          	jra	L3421
2431  0314               L5421:
2432                     ; 655   else if (statusreg == 0x0300) /* The flag to check is in SWIC register */
2434  0314 1e01          	ldw	x,(OFST-2,sp)
2435  0316 a30300        	cpw	x,#768
2436  0319 2607          	jrne	L1521
2437                     ; 657     tmpreg = CLK->SWCR;
2439  031b c650c5        	ld	a,20677
2440  031e 6b03          	ld	(OFST+0,sp),a
2443  0320 2013          	jra	L3421
2444  0322               L1521:
2445                     ; 659   else if (statusreg == 0x0400) /* The flag to check is in CSS register */
2447  0322 1e01          	ldw	x,(OFST-2,sp)
2448  0324 a30400        	cpw	x,#1024
2449  0327 2607          	jrne	L5521
2450                     ; 661     tmpreg = CLK->CSSR;
2452  0329 c650c8        	ld	a,20680
2453  032c 6b03          	ld	(OFST+0,sp),a
2456  032e 2005          	jra	L3421
2457  0330               L5521:
2458                     ; 665     tmpreg = CLK->CCOR;
2460  0330 c650c9        	ld	a,20681
2461  0333 6b03          	ld	(OFST+0,sp),a
2463  0335               L3421:
2464                     ; 668   if ((tmpreg & (uint8_t)CLK_FLAG) != (uint8_t)RESET)
2466  0335 7b05          	ld	a,(OFST+2,sp)
2467  0337 1503          	bcp	a,(OFST+0,sp)
2468  0339 2706          	jreq	L1621
2469                     ; 670     bitstatus = SET;
2471  033b a601          	ld	a,#1
2472  033d 6b03          	ld	(OFST+0,sp),a
2475  033f 2002          	jra	L3621
2476  0341               L1621:
2477                     ; 674     bitstatus = RESET;
2479  0341 0f03          	clr	(OFST+0,sp)
2481  0343               L3621:
2482                     ; 678   return((FlagStatus)bitstatus);
2484  0343 7b03          	ld	a,(OFST+0,sp)
2487  0345 5b05          	addw	sp,#5
2488  0347 81            	ret
2534                     ; 687 ITStatus CLK_GetITStatus(CLK_IT_TypeDef CLK_IT)
2534                     ; 688 {
2535                     	switch	.text
2536  0348               _CLK_GetITStatus:
2538  0348 88            	push	a
2539  0349 88            	push	a
...
2549  034a a11c          	cp	a,#28
2550  034c 2611          	jrne	L7031
2551                     ; 697     if ((CLK->SWCR & (uint8_t)CLK_IT) == (uint8_t)0x0C)
2553  034e c450c5        	and	a,20677
2554  0351 a10c          	cp	a,#12
2555  0353 2606          	jrne	L1131
2556                     ; 699       bitstatus = SET;
2558  0355 a601          	ld	a,#1
2559  0357 6b01          	ld	(OFST+0,sp),a
2562  0359 2015          	jra	L5131
2563  035b               L1131:
2564                     ; 703       bitstatus = RESET;
2566  035b 0f01          	clr	(OFST+0,sp)
2568  035d 2011          	jra	L5131
2569  035f               L7031:
2570                     ; 709     if ((CLK->CSSR & (uint8_t)CLK_IT) == (uint8_t)0x0C)
2572  035f c650c8        	ld	a,20680
2573  0362 1402          	and	a,(OFST+1,sp)
2574  0364 a10c          	cp	a,#12
2575  0366 2606          	jrne	L7131
2576                     ; 711       bitstatus = SET;
2578  0368 a601          	ld	a,#1
2579  036a 6b01          	ld	(OFST+0,sp),a
2582  036c 2002          	jra	L5131
2583  036e               L7131:
2584                     ; 715       bitstatus = RESET;
2586  036e 0f01          	clr	(OFST+0,sp)
2588  0370               L5131:
2589                     ; 720   return bitstatus;
2591  0370 7b01          	ld	a,(OFST+0,sp)
2594  0372 85            	popw	x
2595  0373 81            	ret
2631                     ; 729 void CLK_ClearITPendingBit(CLK_IT_TypeDef CLK_IT)
2631                     ; 730 {
2632                     	switch	.text
2633  0374               _CLK_ClearITPendingBit:
2637                     ; 732   assert_param(IS_CLK_IT_OK(CLK_IT));
2639                     ; 734   if (CLK_IT == (uint8_t)CLK_IT_CSSD)
2641  0374 a10c          	cp	a,#12
2642  0376 2606          	jrne	L1431
2643                     ; 737     CLK->CSSR &= (uint8_t)(~CLK_CSSR_CSSD);
2645  0378 721750c8      	bres	20680,#3
2647  037c 2004          	jra	L3431
2648  037e               L1431:
2649                     ; 742     CLK->SWCR &= (uint8_t)(~CLK_SWCR_SWIF);
2651  037e 721750c5      	bres	20677,#3
2652  0382               L3431:
2653                     ; 745 }
2656  0382 81            	ret
